...
bot.command(/мой баланс/i, async (ctx) => { // Прослушка новых сообщений "мой баланс"
    const result = await vkcoin.getBalance([ ctx.message.from_id ]); // Получаем баланс отправителя
    const coins = vkcoin.formatCoins(result.response[ctx.message.from_id]); // Делаем его читабельным
